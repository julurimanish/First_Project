...
static irqreturn_t irq_handler(int irq,void *dev_id) {
  printk(KERN_INFO "Shared IRQ: Interrupt Occurred for write\n");
  return IRQ_HANDLED;
}
                ...
                const char __user *buf, size_t len, loff_t *off)
{
  ...
  /*if (request_irq(IRQ_NO, irq_handler, IRQF_SHARED, "etx_device1", (void *)(irq_handler))) {
...
static int __init etx_driver_init(void)
{
        ...
        if (request_irq(IRQ_NO, irq_handler,0, "etx_device1", (void *)(irq_handler))) {
            printk(KERN_INFO "my_device: cannot register IRQ ");
                    goto irq;
        }
