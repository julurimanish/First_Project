...
static struct task_struct *wait_thread1;
...
static int wait_function1(void *unused)
{
...
static int __init etx_driver_init(void)
{
	...
	wait_thread1 = kthread_create(wait_function1, NULL, "WaitThread1");
        if (wait_thread) {
                pr_info("Thread-1 Created successfully\n");
                wake_up_process(wait_thread1);
        } else
                pr_err("Thread-1 creation failed\n");
